...
$("#cashDrawer").click(function(){
	...
	$("#diagnosis").click(function(){
		...
		mainTab.tabs("select", newIndex);
			...
			$("#inputStoreDrugs").click(function(){
				if(!tabNameExists("Input Store Drugs")){
			mainTab.tabs('add','inputStoreDrugs.php','Input Store Drugs');
			var newIndex = mainTab.tabs("length") - 1;
			mainTab.tabs("select", newIndex);
				}else{
					var index = getTabName("Input Store Drugs");
					mainTab.tabs('select', index);
				}
		});
	...
	$("#inputLab").click(function(){
		if(!tabNameExists("Add Test")){
		mainTab.tabs('add','inputLab.php','Add Test');
		var newIndex = mainTab.tabs("length") - 1;
		mainTab.tabs("select", newIndex);
		}else{
			var index = getTabName("Add Test");
			mainTab.tabs('select', index);
		}
		});
